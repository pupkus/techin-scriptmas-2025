...
window.onload = () => {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        const firstRowLen = formData.get("row-len");
        const totalRows = formData.get("rows");
        const incrementBy = Number(formData.get("increment-by")) || incBy;
        const incrementEvery = Number(formData.get("increment-every")) || incEvery;
...
async function drawRow(hallContainer, seats, totalRows) {
