import { drawTarget, getTargetState, worldToCanvas, WORLD_EXTENT, TargetState } from "./target.js";
...
function scoreShot(x: number, y: number, state: TargetState): ShotResult {
...
function renderResult(resultsDiv: HTMLDivElement, result: ShotResult) {
  const ringText =
...
export function handleShot(
  ...
  if (options?.random) {
    ...
    // update inputs
    const sx = document.getElementById("shoot-xc") as HTMLInputElement;
    const sy = document.getElementById("shoot-yc") as HTMLInputElement;

    if (sx) sx.value = x.toFixed(2);
    if (sy) sy.value = y.toFixed(2);
  } else {
