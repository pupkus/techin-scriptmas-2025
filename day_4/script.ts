...
window.onload = () => {
  ...
  function addChip(value: number) {
    ...
    newChip.className = `price inline-flex items-center px-3 py-1 rounded-full text-sm font-medium mx-2 ${
      value > 10 ? "bg-teal-900 text-teal-100" : "bg-red-900 text-red-100"
    }`;
  ...
  function finish() {
    const priceChips = Array.from(document.querySelectorAll<HTMLElement>(".price")).filter(
      ({ innerText }) => Number(innerText) <= 10
    );
