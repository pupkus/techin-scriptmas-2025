...
window.onload = () => {
  ...
  const calculationMinuteArrow = document.querySelector(".minute") as HTMLDivElement;
  const calculationHourArrow = document.querySelector(".hour") as HTMLDivElement;
  ...
  function addTime(currentTime: number, currentPost: { hours: number; minutes: number } = { hours: 0, minutes: 0 }) {
